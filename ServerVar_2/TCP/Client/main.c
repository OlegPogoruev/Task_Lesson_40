...
//    server.sin_addr.s_addr = inet_addr("127.0.0.1");
	...
	if( (recv_len = recv(sock_client , buf_receiv , BUFLEN , 0)) < 0){perror("recv failed");return (-1);}
	buf_receiv[recv_len] = 0;
	close(sock_client);



	(*((char*)&(new_port) + 0)) = buf_receiv[0];
	(*((char*)&(new_port) + 1)) = buf_receiv[1];
	printf("buf_receiv[0] = %d\n", buf_receiv[0]);
	printf("buf_receiv[1] = %d\n", buf_receiv[1]);
	printf("new_port = %d\n", new_port);
	if(0 == new_port)
	{printf("Server Busy\n");return 0;}
